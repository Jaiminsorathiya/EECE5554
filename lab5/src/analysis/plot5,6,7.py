...
# Function to load sensor data from ROS bag file
...
# Function to calculate velocity from accelerometer data
def calculate_velocity(accel_data, dt):
    velocity = np.cumsum(accel_data) * dt
...
# Function to calculate trajectory from velocity and yaw
def calculate_trajectory(velocity, yaw, dt):
    displacement_x = np.cumsum(velocity * np.cos(yaw)) * dt
    displacement_y = np.cumsum(velocity * np.sin(yaw)) * dt
...
imu_topic = '/imu'
gps_topic = '/gps'
dt = 1.0 / 50  # Assumed sampling rate
...
velocity_imu = calculate_velocity(accel_x, dt)
...
yaw = np.cumsum(imu_data['imu.angular_velocity.z'].values) * dt
x_traj, y_traj = calculate_trajectory(velocity_imu, yaw, dt)
